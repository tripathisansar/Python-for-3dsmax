...
class ProjectInfo():
    ...
    def getShotNumber():
        if (ProjectInfo.getShotIndex() == False):
            return "ERRR"
        else:
...
class SMTD_sanityCheck():
    ...
    def loadSanityCheck(self):
        ...
        sanityCheckDrv = r'B:\__Pipeline Tools\Scene Archiver\01_SanityChecks' + path.sep
...
class Worker(QThread):
    ...
    """  Deprecated function - see ProjectInfo class   
        ...
        if path.isdir(archiveDir):
            ...
            if not path.isfile(zipFilename):
                ...
                with ZipFile(zipFilename, 'w', zipfile.ZIP_DEFLATED) as zip:
                    ...
                    self.statusText.emit(path.basename(maxfile))
