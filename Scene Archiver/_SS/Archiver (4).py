...
class SMTD_sanityCheck():
    ...
    def loadSanityCheck(self):
        ...
        if len(rt.SMTD_sanityChecksToPerform)==0:
...
class SceneArchiver(qw.QDialog):
    ...
    def init_Info(self):
        ...
        if (str(rt.renderers.current)[0:5]) == 'V_Ray' or (str(rt.renderers.current)[0:5]) == 'Coron':
            ...
            self.outputPath.setText('Asset Path: ' + projectInfo.getAssetPath())
        else:
    ...
    def test(self):
        try:
            if rt.SMTD_Private_sanityChecksToPerform:
                print('loaded')
        except:
            print('not Loaded')
